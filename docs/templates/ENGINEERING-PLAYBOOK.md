...
每完成一个 Stage（一批 Issue 全部合并、分支同步完成后），CTO 必须执行以下回顾流程：
...
在 `docs/SPRINT-LOG.md` 中记录，格式如下：

```markdown
### Stage X 回顾（YYYY-MM-DD）

**完成的任务**: 列出所有合并的 Issue/PR
**遇到的问题**:
- 问题1 → 根因 → 改进措施
**框架调整**:
- 调整1：原因和变更内容
...
- Stage 内所有 PR 合并完成
- 分支同步（main ↔ develop）完成
- 回顾记录写入 SPRINT-LOG.md 后，才能开始下一个 Stage
